...
| 1     | Squelette Maven multi-module (core/tenant/…)       | À venir      |
...
POM parent avec BOMs (Vert.x, Jackson, Postgres, JUnit5, REST-assured, Spotless, Checkstyle).
Modules : `core`, `tenant`, `identity`, `authz`, `oauth`, `api`. Module `core` minimal
avec `MainVerticle` placeholder.
Vérifications : `mvn verify` passe, Spotless/Checkstyle s'exécutent.
...
- **Lint** : Spotless (formatage) + Checkstyle (style) via `mvn spotless:apply`
